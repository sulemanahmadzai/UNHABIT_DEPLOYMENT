...
def _fallback_plan21(quiz_summary: Optional[QuizSummary] = None) -> Plan21D:
    ...
    day_tasks = {
        "day_1": [
            DayTask(title="Track patterns", description=f"Write down when and why {habit} happens.", reason="Understanding your patterns is the first step to breaking them.", kind="reflection"),
            DayTask(title="Identify trigger", description=f"Note what most often leads to {habit}.", reason="Knowing your triggers helps you prepare and avoid automatic responses.", kind="cognitive"),
            DayTask(title="Set intention", description=f"Write why reducing {habit} matters.", reason="Clear motivation strengthens your commitment when urges arise.", kind="identity"),
        ],
        "day_2": [
            DayTask(title="Pause before acting", description=f"Before {habit}, pause 30 seconds.", reason="This breaks the automatic trigger-response loop and creates space for choice.", kind="cognitive"),
            DayTask(title="Create distance", description=f"Move away from {trigger} location.", reason="Physical distance weakens the cue-craving connection in your brain.", kind="environmental"),
            DayTask(title="Choose alternative", description="Pick a 5-minute healthy activity.", reason="Building alternative dopamine sources reduces dependence on the habit.", kind="behavioral"),
        ],
        "day_3": [
            DayTask(title="Remove one cue", description=f"Disable one cue that feeds {habit}.", reason="Eliminating triggers makes it harder for the habit to start automatically.", kind="environmental"),
            DayTask(title="Practice urge surfing", description="Breathe and observe urges without acting.", reason="Learning that urges pass without action builds confidence and control.", kind="cognitive"),
            DayTask(title="Delay once", description=f"Delay {habit} by 5 minutes today.", reason="Delaying proves you have control and weakens the urgency of cravings.", kind="behavioral"),
        ],
        "day_4": [
            DayTask(title="Set cutoff time", description=f"Choose when to stop {habit} today.", reason="Clear boundaries create structure and reduce decision fatigue.", kind="behavioral"),
            DayTask(title="Change environment", description=f"Rearrange your {habit} location.", reason="Disrupting familiar settings breaks automatic behavior patterns.", kind="environmental"),
            DayTask(title="Review motivation", description=f"Re-read why {motive} matters.", reason="Reconnecting with your why strengthens resolve during difficult moments.", kind="identity"),
        ],
        "day_5": [
            DayTask(title="Reduce by half", description=f"Cut one {habit} episode in half.", reason="Gradual reduction builds confidence while avoiding overwhelming withdrawal.", kind="behavioral"),
            DayTask(title="Create trigger-free zone", description="Designate one space as off-limits.", reason="Protected spaces give you safe zones where the habit can't follow.", kind="environmental"),
            DayTask(title="Practice replacement", description="Do alternative at high-risk time.", reason="Replacing the habit at peak times rewires your reward system.", kind="behavioral"),
        ],
        "day_6": [
            DayTask(title="Plan evening", description="Create routine without main trigger.", reason="Structured routines reduce exposure to high-risk situations.", kind="behavioral"),
            DayTask(title="Strengthen rule", description="Make cutoff 30 minutes earlier.", reason="Progressive tightening builds discipline and shrinks the habit window.", kind="behavioral"),
            DayTask(title="Reflect on progress", description="Note one win and one challenge.", reason="Acknowledging progress maintains motivation and identifies areas to improve.", kind="reflection"),
        ],
        "day_7": [
            DayTask(title="Weekly review", description="Review week and adjust approach.", reason="Regular assessment helps you learn what works and adapt your strategy.", kind="reflection"),
            DayTask(title="Celebrate wins", description="Acknowledge any progress made.", reason="Celebrating success reinforces positive identity change and builds momentum.", kind="identity"),
            DayTask(title="Plan week 2", description="Identify biggest challenge and strategy.", reason="Proactive planning prepares you to handle obstacles before they arise.", kind="cognitive"),
        ],
        "day_8": [
            DayTask(title="Increase delay", description=f"Delay {habit} by 10 minutes today.", reason="Longer delays strengthen your ability to tolerate discomfort and urges.", kind="behavioral"),
            DayTask(title="Change location", description=f"Do {habit} somewhere less comfortable.", reason="Making the habit inconvenient reduces its automatic appeal.", kind="environmental"),
            DayTask(title="Write to future you", description=f"Explain why reducing {habit} matters.", reason="Connecting with your future self strengthens long-term thinking over impulse.", kind="identity"),
        ],
        "day_9": [
            DayTask(title="Create no-go rule", description=f"Define where {habit} is not allowed.", reason="Clear rules eliminate negotiation with yourself in weak moments.", kind="behavioral"),
            DayTask(title="Prepare environment", description="Set up trigger-free first hour tomorrow.", reason="Morning success sets a positive tone and builds daily momentum.", kind="environmental"),
            DayTask(title="Urge surfing again", description="Let one urge pass unacted.", reason="Repeated practice proves urges are temporary and manageable.", kind="cognitive"),
        ],
        "day_10": [
            DayTask(title="Full replacement", description=f"Replace one {habit} episode completely.", reason="Complete replacement demonstrates you can meet needs without the habit.", kind="behavioral"),
            DayTask(title="Remove access", description=f"Add barrier to {habit} access.", reason="Friction points give you time to reconsider before acting impulsively.", kind="environmental"),
            DayTask(title="Reflect on identity", description="Complete: 'I am becoming someone who...'", reason="Identity shifts create internal motivation stronger than willpower alone.", kind="identity"),
        ],
        "day_11": [
            DayTask(title="Handle slip mindfully", description="Review any slip analytically.", reason="Learning from setbacks prevents shame spirals and builds resilience.", kind="reflection"),
            DayTask(title="Strengthen morning", description="Create morning routine for success.", reason="Strong mornings create positive momentum that carries through the day.", kind="behavioral"),
            DayTask(title="Teach someone", description=f"Share insight about {habit} triggers.", reason="Teaching solidifies your understanding and reinforces your commitment.", kind="cognitive"),
        ],
        "day_12": [
            DayTask(title="Extend cutoff", description=f"Make no-{habit} window 1 hour longer.", reason="Expanding restriction zones progressively shrinks the habit's territory.", kind="behavioral"),
            DayTask(title="Create accountability", description="Tell someone about your progress.", reason="External accountability adds social motivation to internal commitment.", kind="identity"),
            DayTask(title="Plan for high-risk", description="Identify top 3 high-risk situations.", reason="Anticipating challenges lets you prepare coping strategies in advance.", kind="cognitive"),
        ],
        "day_13": [
            DayTask(title="Practice prevention", description=f"Do something before high-risk time.", reason="Proactive action prevents the habit before cravings intensify.", kind="behavioral"),
            DayTask(title="Redesign space", description="Make permanent environmental change.", reason="Lasting changes create ongoing support without requiring daily willpower.", kind="environmental"),
            DayTask(title="Reflect on costs", description=f"Write costs of continuing {habit}.", reason="Awareness of consequences strengthens motivation to maintain change.", kind="reflection"),
        ],
        "day_14": [
            DayTask(title="Mid-point review", description="List 3 wins and 1 adjustment.", reason="Halfway reflection builds confidence and refines your approach.", kind="reflection"),
            DayTask(title="Celebrate progress", description="Acknowledge how far you've come.", reason="Recognition of growth reinforces your new identity and sustains effort.", kind="identity"),
            DayTask(title="Commit to week 3", description="Write commitment to finish strong.", reason="Renewed commitment prevents mid-journey dropout and maintains focus.", kind="identity"),
        ],
        "day_15": [
            DayTask(title="Implement no-go rule", description=f"Enforce no-{habit} rule strictly.", reason="Strict enforcement builds self-trust and eliminates negotiation.", kind="behavioral"),
            DayTask(title="Add second rule", description="Define second off-limits context.", reason="Multiple boundaries create comprehensive protection against triggers.", kind="behavioral"),
            DayTask(title="Visualize success", description="Visualize handling biggest trigger.", reason="Mental rehearsal prepares your brain to respond effectively under pressure.", kind="cognitive"),
        ],
        "day_16": [
            DayTask(title="Go trigger-free", description="Avoid main trigger for 24 hours.", reason="Complete avoidance proves you can function without the trigger.", kind="behavioral"),
            DayTask(title="Strengthen identity", description="Update identity statement with evidence.", reason="Evidence-based identity change is more powerful than aspirational thinking.", kind="identity"),
            DayTask(title="Plan maintenance", description="Think about post-day-21 maintenance.", reason="Long-term planning prevents relapse after the structured program ends.", kind="cognitive"),
        ],
        "day_17": [
            DayTask(title="Handle with confidence", description="Remind yourself you can handle urges.", reason="Self-efficacy beliefs directly predict success in behavior change.", kind="cognitive"),
            DayTask(title="Make it permanent", description="Choose one change to keep forever.", reason="Permanent commitments shift from temporary effort to lasting lifestyle.", kind="environmental"),
            DayTask(title="Reflect on growth", description="Write about how you've changed.", reason="Documenting transformation reinforces new identity and prevents backsliding.", kind="reflection"),
        ],
        "day_18": [
            DayTask(title="Test your strength", description=f"Expose to mild trigger without acting.", reason="Controlled exposure builds confidence that you can resist in real situations.", kind="behavioral"),
            DayTask(title="Refine system", description="Adjust rules that aren't working well.", reason="Optimization ensures your system remains effective and sustainable.", kind="cognitive"),
            DayTask(title="Share learning", description=f"Teach key insight about {habit}.", reason="Teaching others deepens your mastery and strengthens commitment.", kind="identity"),
        ],
        "day_19": [
            DayTask(title="Create identity statement", description="Write 2-sentence identity statement.", reason="Clear identity statements guide decisions when motivation fluctuates.", kind="identity"),
            DayTask(title="Plan long-term", description="Decide which 3 rules to keep permanently.", reason="Selecting core rules creates sustainable structure without overwhelming complexity.", kind="cognitive"),
            DayTask(title="Prepare for challenges", description="Plan for next major trigger.", reason="Preparation prevents panic and ensures you have strategies ready.", kind="behavioral"),
        ],
        "day_20": [
            DayTask(title="Keystone rule", description="Choose one non-negotiable rule.", reason="A single unbreakable rule creates a foundation for all other changes.", kind="behavioral"),
            DayTask(title="Reflect on transformation", description="Compare day 1 you vs. today you.", reason="Recognizing transformation validates effort and builds confidence for the future.", kind="reflection"),
            DayTask(title="Plan day 22", description="Decide what to do on day 22.", reason="Planning beyond the program prevents the 'finish line' relapse effect.", kind="cognitive"),
        ],
        "day_21": [
            DayTask(title="Review journey", description="Read notes from day 1.", reason="Seeing your progress from start to finish reinforces how far you've come.", kind="reflection"),
            DayTask(title="Refresh identity", description="Finalize identity statement.", reason="A clear identity statement guides future decisions and prevents regression.", kind="identity"),
            DayTask(title="Set long-term vision", description=f"Write 90-day vision for {habit}.", reason="Long-term vision ensures this is the beginning of lasting change, not the end.", kind="cognitive"),
            DayTask(title="Celebrate completion", description="Acknowledge your success.", reason="Celebration reinforces positive identity and marks this as a significant life achievement.", kind="identity"),
        ],
    }
