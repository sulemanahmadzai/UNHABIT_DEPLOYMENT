...
export async function getDashboard(userId) {
    ...
    const [activeJourney, streaks, pointBalance, todayXP, buddyCount, buddiesCompletedToday, earnedBadgesCount, nextBadge, profile,] = await Promise.all([
 ...
 */
async function getActiveJourneyWithToday(userId) {
 ...
 */
async function getTodaysTasks(userId, journeyId, dayNumber) {
 ...
 * Get today's XP
 */
async function getTodayXP(userId) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
 ...
 * Get buddy count
 */
async function getBuddyCount(userId) {
 ...
 */
async function getBuddiesCompletedToday(userId) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
 ...
 */
async function calculateHabitHealth(userId, journeyId) {
