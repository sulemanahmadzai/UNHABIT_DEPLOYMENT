...
function isInQuietHours(minuteOfDay, start, end) {
    if (start === end)
        return false;
    // Non-wrapping window
    if (start < end)
...
function isInPrimeTime(minuteOfDay, windows) {
    if (windows.length === 0)
        return true; // no prime time configured => allow
...
async function getUserDeliveryGate(userId, now) {
    ...
    if (!isInPrimeTime(minuteOfDay, prime)) {
        return { allowed: false, reason: "outside_prime_time" };
    }
...
export async function processDueTaskReminders(now, logger = console) {
    ...
    for (const r of reminders) {
        ...
        const res = await sendPushNotifications(tokens, "Task Reminder", `Don't forget: ${r.journey_tasks.title}`, {
...
export async function processDueScheduledNudges(now, logger = console) {
...
export async function processPushReceipts(now, logger = console) {
    ...
    for (const d of deliveries) {
        const meta = (d.meta ?? {});
        if (meta.receipts_checked)
            continue;
        const pairs = Array.isArray(meta.receipt_pairs) ? meta.receipt_pairs : [];
        const filtered = pairs.filter((p) => p?.token && p?.receiptId);
        if (filtered.length)
            toCheck.push({ deliveryId: d.id, pairs: filtered });
    }
    ...
    for (const item of toCheck) {
        ...
        await db.nudge_deliveries.update({
            ...
            data: {
                meta: {
                    ...deliveries.find((d) => d.id === item.deliveryId)?.meta,
...
export function startNotificationCron(options = {}) {
