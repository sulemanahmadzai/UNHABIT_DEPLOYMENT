...
async function getCron() {
    try {
        ...
        console.warn("[cron] node-cron is not installed. Cron jobs are disabled.\n" +
            "  Run: npm install node-cron && npm install --save-dev @types/node-cron");
        return null;
    }
}
function getUserLocalMinute(now, timezone) {
    try {
...
function isInWindow(minuteOfDay, target, windowMinutes = 5) {
    return minuteOfDay >= target && minuteOfDay < target + windowMinutes;
}
// ─── Main tick: runs every 5 minutes ───────────────────────────────
async function processTimezoneAwareScenarios(now, logger) {
    const profiles = await db.profiles.findMany({
        select: { user_id: true, timezone: true },
    });
    const activeJourneyUsers = new Set((await db.journeys.findMany({
        where: { status: "active" },
        select: { user_id: true },
        distinct: ["user_id"],
    })).map((j) => j.user_id));
    ...
    for (const profile of profiles) {
        if (!activeJourneyUsers.has(profile.user_id))
            continue;
        const tz = profile.timezone || "UTC";
        const { minuteOfDay, dow } = getUserLocalMinute(now, tz);
        const settings = settingsMap.get(profile.user_id);
        const morningMinute = settings?.morning_checkin_minute ?? 480; // 8:00 AM
        const eveningMinute = settings?.evening_lastcall_minute ?? 1260; // 9:00 PM
...
async function processStreakAtRisk(logger) {
...
async function processMissedDayRecovery(logger) {
...
async function processTwoMissRisk(logger) {
...
async function processBuddyNudgePrompts(logger) {
    const targets = await Scenarios.getBuddyNudgeTargets();
    let sent = 0;
    for (const { userId, buddyName } of targets) {
        const result = await Scenarios.notifyNudgeYourBuddy(userId, buddyName ?? undefined, logger);
        if (result)
            sent++;
    }
...
async function processCoachDailyCheckin(logger) {
    const profiles = await db.profiles.findMany({
        select: { user_id: true, timezone: true },
    });
    const now = new Date();
    const allCoachSettings = await db.notification_settings.findMany();
    const settingsMap2 = new Map(allCoachSettings.map((s) => [s.user_id, s]));
    let sent = 0;
    for (const profile of profiles) {
        const tz = profile.timezone || "UTC";
        const { minuteOfDay } = getUserLocalMinute(now, tz);
        const settings = settingsMap2.get(profile.user_id);
        const morningMinute = settings?.morning_checkin_minute ?? 480;
...
async function processCoachReflectionPrompt(logger) {
...
async function processLeaderboardWeeklyStart(logger) {
...
async function processWeeklyRewardSummary(logger) {
    const today = new Date();
    ...
    for (const { user_id } of activeJourneys) {
        ...
        const result = await Scenarios.notifyWeeklyRewardSummary(user_id, daysCompleted, xpEarned, logger);
        if (result)
            sent++;
    }
...
async function processTrialEndingSoon(logger) {
...
async function processPlanExpiration(logger) {
...
async function processBuddyInactivity(logger) {
    ...
    for (const link of activeLinks) {
        ...
        for (const [userId, buddyId] of pairs) {
            ...
            if (recentActivity === 0) {
                ...
                const result = await Scenarios.notifyBuddyInactivity(userId, buddyProfile?.full_name ?? undefined, logger);
                if (result)
                    sent++;
            }
        }
...
async function processCoachStuckDetection(logger) {
    const users = await Scenarios.getUsersWithTwoMissRisk();
    let sent = 0;
    for (const userId of users) {
...
async function processPhaseTransition(logger) {
...
async function processNextBadgeProgress(logger) {
    const activeJourneys = await db.journeys.findMany({
        where: { status: "active" },
        select: { user_id: true },
        distinct: ["user_id"],
    ...
    for (const { user_id } of activeJourneys) {
        ...
        for (const rule of badgeRules) {
            if (earnedSet.has(rule.badge_id))
                continue;
...
async function processCalendarPlanPrompt(logger) {
    const yesterday = new Date();
    yesterday.setDate(yesterday.getDate() - 1);
...
async function processLeaderboardRankChange(logger) {
    const activeJourneys = await db.journeys.findMany({
        where: { status: "active" },
        select: { user_id: true },
        distinct: ["user_id"],
...
async function processStreakFreezeOffered(logger) {
...
// ═══════════════════════════════════════════════════════════════════
export async function startCronJobs(logger = console) {
