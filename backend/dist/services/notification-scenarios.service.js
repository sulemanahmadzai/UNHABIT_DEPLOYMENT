...
import { getNotificationCopy, getPrivacySafeCopy } from "./notification-templates.service.js";
 ...
 */
async function sendScenarioNotification(userId, category, scenario, opts = {}, logger = console) {
    try {
        const gate = await checkDeliveryGate(userId, category, scenario, opts.isBillingOrSecurity ?? false);
        if (!gate.allowed) {
 ...
 */
export async function notifyDailyCheckinReady(userId, logger) {
 ...
 */
export async function notifyTaskReminder(userId, taskTitle, logger) {
    const completed = await hasCompletedToday(userId);
    if (completed)
        return false;
 ...
 */
export async function notifyMiddayRescue(userId, logger) {
    const completed = await hasCompletedToday(userId);
    if (completed)
        return false;
 ...
 */
export async function notifyEveningLastCall(userId, logger) {
    const completed = await hasCompletedToday(userId);
    if (completed)
        return false;
 ...
 */
export async function notifyCompletionReinforcement(userId, streakLength) {
 ...
 */
export async function notifyMicroStreak(userId, streakLength) {
    if (streakLength < 2)
        return false;
 ...
 */
export async function notifyDayResetCleanSlate(userId, logger) {
 ...
 */
export async function notifyOpenAppNudge(userId, logger) {
    const completed = await hasCompletedToday(userId);
    if (completed)
        return false;
 ...
 */
export async function notifyHabitHealthChange(userId) {
 ...
 */
export async function notifyCalendarPlanPrompt(userId) {
 ...
 */
export async function notifyStreakAtRisk(userId, streakLength, logger) {
 ...
 */
export async function notifyMissedDayRecovery(userId, logger) {
 ...
 */
export async function notifyTwoMissRisk(userId, logger) {
 ...
 */
export async function notifyRelapsLogged(userId) {
 ...
 * Opt-in only.
 */
export async function notifyHighRiskWindow(userId, logger) {
    const settings = await db.notification_settings.findUnique({ where: { user_id: userId } });
    if (!settings?.high_risk_reminders)
        return false;
 ...
 */
export async function notifyWeekendSupport(userId, logger) {
    const settings = await db.notification_settings.findUnique({ where: { user_id: userId } });
    if (!settings?.weekend_support)
        return false;
 ...
 */
export async function notifyStreakFreezeOffered(userId) {
 ...
 */
export async function notifyStreakFreezeUsed(userId) {
 ...
 */
export async function notifyStreakMilestone(userId, streakLength) {
 ...
 */
export async function notifyPost21Maintenance(userId) {
 ...
 */
export async function notifyCoachReply(userId, sessionId) {
 ...
 */
export async function notifyCoachDailyCheckin(userId, logger) {
 ...
 */
export async function notifyCoachStuckDetection(userId, logger) {
 ...
 */
export async function notifyCoachPhaseTransition(userId, dayNumber) {
 ...
 */
export async function notifyCoachSkillSuggestion(userId) {
 ...
 */
export async function notifyCoachReflectionPrompt(userId, logger) {
 ...
 */
export async function notifyBuddyInviteReceived(userId) {
 ...
 */
export async function notifyBuddyInviteAccepted(inviterId, buddyName) {
 ...
 */
export async function notifyBuddyCompletedToday(userId, buddyName) {
 ...
 */
export async function notifyBuddyStreakMilestone(userId, buddyName, streakLength) {
 ...
 */
export async function notifyNudgeYourBuddy(userId, buddyName, logger) {
 ...
 */
export async function notifyLeaderboardWeeklyStart(userId, logger) {
 ...
 */
export async function notifyLeaderboardRankChange(userId, rankChange, logger) {
 ...
 * Trigger: cron (weekly).
 */
export async function notifyBuddyInactivity(userId, buddyName, logger) {
 ...
 */
export async function notifyXpEarned(userId, xpAmount) {
    if (xpAmount < 10)
        return false; // Only notify for meaningful XP
 ...
 */
export async function notifyLevelUp(userId, level) {
 ...
 */
export async function notifyBadgeUnlocked(userId, badgeName) {
 ...
 */
export async function notifyNextBadgeProgress(userId, badgeName, logger) {
 ...
 */
export async function notifyWeeklyRewardSummary(userId, daysCompleted, xpEarned, logger) {
 ...
 */
export async function notifySharePrompt(userId) {
 ...
 */
export async function notifyTrialStarted(userId) {
 ...
 */
export async function notifyTrialEndingSoon(userId, daysRemaining, logger) {
 ...
 */
export async function notifySubscriptionRenewed(userId) {
 ...
 * Trigger: event (from stripe webhook handler).
 */
export async function notifyBillingFailure(userId) {
 ...
 */
export async function notifyPlanExpiration(userId, logger) {
 ...
 */
export async function notifyPromotionalOffer(userId, logger) {
 ...
 */
export async function getUsersWithTrialEndingSoon(withinDays) {
    const now = new Date();
    ...
    return subs.map((s) => ({
        userId: s.user_id,
        daysRemaining: Math.ceil(((s.trial_end?.getTime() ?? now.getTime()) - now.getTime()) / (1000 * 60 * 60 * 24)),
    }));
}
//# sourceMappingURL=notification-scenarios.service.js.map