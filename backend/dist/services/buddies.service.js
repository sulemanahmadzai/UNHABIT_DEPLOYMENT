 ...
 */
export async function getQuickView(userId) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    // Get all buddy links
    const links = await db.buddy_links.findMany({
    ...
    const buddyIds = links.map(link => link.user_a === userId ? link.user_b : link.user_a);
    // Get buddies who completed tasks today
 ...
 */
export async function getBuddyProfile(userId, buddyLinkId) {
    // Verify user is part of this buddy link
    const link = await db.buddy_links.findFirst({
        where: {
            id: buddyLinkId,
    ...
    if (activeJourney && activeJourney.start_date) {
        ...
        if (todaysTasks) {
            const allCompleted = todaysTasks.journey_tasks.every(t => t.user_task_progress.some(p => p.status === "completed"));
    ...
    if (activeJourney && activeJourney.start_date) {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            if (dayData && dayData.journey_tasks.length > 0) {
                const allCompleted = dayData.journey_tasks.every(t => t.user_task_progress.some(p => p.status === "completed"));
...
function getLevelName(level) {
    ...
    if (level <= 10)
        return names[3] || "Champion";
    if (level <= 20)
        return names[4] || "Master";
    if (level <= 50)
        return names[5] || "Legend";
    return names[6] || "Elite";
}
/**
 * Send a quick nudge to buddy
 */
export async function sendNudge(userId, buddyLinkId, message) {
    // Verify user is part of this buddy link
    const link = await db.buddy_links.findFirst({
        where: {
            id: buddyLinkId,
    ...
    try {
        ...
        const tokens = await db.devices
            ...
            .then((rows) => Array.from(new Set(rows.map((r) => r.push_token).filter((t) => !!t))));
        if (tokens.length) {
 ...
 * Get nudges received
 */
export async function getNudges(userId, limit = 20) {
 ...
 * Resend an invite
 */
export async function resendInvite(userId, inviteId) {
    const invite = await db.buddy_invites.findFirst({
        where: {
            id: inviteId,
            inviter_id: userId,
 ...
 * Get invite URL
 */
export function getInviteUrl(inviteCode, baseUrl) {
 ...
 */
export async function getBuddiesCompletedToday(userId) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    // Get all buddy IDs
    const links = await db.buddy_links.findMany({
