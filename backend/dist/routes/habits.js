...
r.post("/:id/triggers", requireAuth, async (req, res, next) => {
    try {
        ...
        const result = await HabitsService.addTriggerToHabit(req.user.id, habitId, trigger_id);
        if (!result) {
...
r.delete("/:id/triggers/:triggerId", requireAuth, async (req, res, next) => {
    try {
        ...
        const removed = await HabitsService.removeTriggerFromHabit(req.user.id, habitId, triggerId);
        if (!removed) {
