...
import type { journey_status } from "@prisma/client";
 ...
 */
export async function getUserJourneys(userId: string, status?: string) {
  return db.journeys.findMany({
    where: {
      user_id: userId,
      ...(status && { status: status as journey_status }),
    },
    include: {
      user_habits: {
        select: {
 ...
 */
export async function createJourney(userId: string, data: CreateJourneyData) {
        ...
        create: data.plan_data.days.map((day) => ({
          ...
          journey_tasks: {
            create: day.tasks.map((task) => ({
              ...
              meta: task.meta ? (task.meta as Prisma.InputJsonValue) : Prisma.JsonNull,
            })),
          },
        })),
      },
...
export async function updateJourney(
  ...
  return db.journeys.update({
    where: { id: journeyId },
    data: {
      ...(data.status && { status: data.status as journey_status }),
 ...
 */
export async function getJourneyDays(userId: string, journeyId: string) {
  const journey = await db.journeys.findFirst({
    where: {
      id: journeyId,
      user_id: userId,
 ...
 * Pause a journey
 */
export async function pauseJourney(userId: string, journeyId: string) {
  const journey = await db.journeys.findFirst({
    where: {
      id: journeyId,
      user_id: userId,
 ...
 * Resume a journey
 */
export async function resumeJourney(userId: string, journeyId: string) {
  const journey = await db.journeys.findFirst({
    where: {
      id: journeyId,
      user_id: userId,
 ...
 */
export async function getActiveJourney(userId: string) {
  return db.journeys.findFirst({
    where: {
      user_id: userId,
      status: "active",
 ...
 */
export async function getTodayJourneyDay(userId: string, journeyId: string) {
  const journey = await db.journeys.findFirst({
    where: {
      id: journeyId,
      user_id: userId,
 ...
 */
export async function restartJourney(userId: string, journeyId: string) {
  const journey = await db.journeys.findFirst({
    where: {
      id: journeyId,
      user_id: userId,
 ...
 */
export async function getJourneyCalendar(userId: string, journeyId: string) {
  const journey = await db.journeys.findFirst({
    where: {
      id: journeyId,
      user_id: userId,
  ...
  const calendarDays = journey.journey_days.map(day => {
    ...
    const completedTasks = day.journey_tasks.filter(
      t => t.user_task_progress.some(p => p.status === "completed")
    ).length;

    let status: "completed" | "partial" | "missed" | "upcoming" | "today" = "upcoming";
