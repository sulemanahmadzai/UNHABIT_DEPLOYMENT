 ...
 */
export async function getHabitHealth(userId: string) {
...
// Helper functions
async function getFreezesUsedThisWeek(userId: string): Promise<number> {
  const today = new Date();
...
function getHealthMessage(score: number): string {
  if (score >= 80) return "Excellent! You're building a strong habit.";
  if (score >= 60) return "Good progress! Keep pushing forward.";
  if (score >= 40) return "You're making progress. Try to be more consistent.";
