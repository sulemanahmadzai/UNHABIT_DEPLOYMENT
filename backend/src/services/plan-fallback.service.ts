...
const t = (
  ...
  reason: string,
  kind: DayTask["kind"]
): DayTask => ({ title, description, reason, kind });

export function buildFallbackPlan(ctx: FallbackContext = {}): FallbackPlan {
  ...
  const plan_summary =
    `This 21-day plan helps you reduce ${habit} with small daily actions, ` +
    `focusing on awareness, friction around ${trigger}, and identity shifts based on ${motive}.`;

  const day_tasks: Record<string, DayTask[]> = {
    day_1: [
 ...
 */
export function extractFallbackContext(quizSummary: unknown, habitGoal?: string): FallbackContext {
  if (!quizSummary || typeof quizSummary !== "object") {
    return { habit: habitGoal };
  }
  const q = quizSummary as Record<string, unknown>;
