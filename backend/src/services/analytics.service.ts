 ...
 */
export async function getIdentityScoreHistory(userId: string, days: number) {
 ...
 */
export async function getConsistencyIndex(userId: string, timeWindow: string) {
 ...
 */
export async function getAdherenceScores(userId: string, journeyId: string) {
 ...
 */
export async function getInsights(userId: string, limit: number) {
 ...
 */
export async function getTriggerHeatmap(userId: string, days: number) {
  const startDate = subDays(new Date(), days);
 ...
 * Get daily metrics
 */
export async function getDailyMetrics(userId: string, days: number) {
  const startDate = subDays(new Date(), days);
 ...
 */
export async function getMissedDaysSummary(userId: string) {
  ...
  for (let d = new Date(startDate); d <= today; d.setDate(d.getDate() + 1)) {
    ...
    if (dayData && dayData.journey_tasks.length > 0) {
      ...
      const allCompleted = dayData.journey_tasks.every(
        task => task.user_task_progress.some(p => p.status === "completed")
      );
  ...
  let lastMissedDaysAgo: number | null = null;
  if (lastMissedDate) {
 ...
 */
export async function getHabitHealthTrend(userId: string, days: number = 7) {
  const activeJourney = await db.journeys.findFirst({
    where: { user_id: userId, status: "active" },
    include: {
      journey_days: {
