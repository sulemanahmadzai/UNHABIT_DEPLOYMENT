...
export function getCategoryDefinition(categoryId: string): NotificationCategory | undefined {
  return CATEGORY_MAP.get(categoryId);
}

export function getAllCategories(): NotificationCategory[] {
 ...
 */
export async function getUserCategoryPrefs(userId: string) {
  const existing = await db.notification_category_prefs.findMany({
    where: { user_id: userId },
  });

  const existingMap = new Map(existing.map((p) => [p.category, p]));
  const result: Array<{ category: string; enabled: boolean; tier: string; name: string }> = [];
 ...
 */
export async function updateCategoryPref(userId: string, category: string, enabled: boolean) {
  const def = CATEGORY_MAP.get(category);
  if (!def) throw new Error(`Unknown notification category: ${category}`);
 ...
 */
export async function bulkUpdateCategoryPrefs(
  userId: string,
...
export async function isCategoryEnabled(userId: string, category: string): Promise<boolean> {
  const def = CATEGORY_MAP.get(category);
  if (!def) return false;
  if (def.tier === "required") return true;
