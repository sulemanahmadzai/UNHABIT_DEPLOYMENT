...
const PRIVACY_SAFE_TEMPLATES: Record<ScenarioKey, (opts: TemplateOptions) => NotificationCopy> = {
 ...
 */
const DETAILED_OVERRIDES: Partial<
  Record<ScenarioKey, (opts: TemplateOptions) => NotificationCopy>
> = {
 ...
 */
export async function getNotificationCopy(
  userId: string,
 ...
 */
export function getPrivacySafeCopy(
  scenario: ScenarioKey,
  opts: TemplateOptions = {}
): NotificationCopy {
  const template = PRIVACY_SAFE_TEMPLATES[scenario];
  if (!template) return { title: "UnHabit", body: "You have a new notification." };
  return template(opts);
}
 ...
 */
export function getDetailedCopy(
  scenario: ScenarioKey,
  opts: TemplateOptions = {}
): NotificationCopy {
