 ...
 */
export async function getUserTaskProgress(userId: string, journeyId?: string) {
  const where: Prisma.user_task_progressWhereInput = {
    user_id: userId,
  };
...
export async function getJourneyProgressSummary(
  ...
  for (const day of journey.journey_days) {
    ...
    completedTasks += day.journey_tasks.filter(
  ...
  const reflectionRate =
    totalDays > 0 ? (daysWithReflection / totalDays) * 100 : 0;
  ...
  return {
    ...
    days: journey.journey_days.map((day) => ({
      ...
      tasks_completed: day.journey_tasks.filter((t) =>
        t.user_task_progress.some((p) => p.status === "completed")
      ).length,
 ...
 */
export async function submitReflection(userId: string, data: ReflectionData) {
 ...
 */
export async function getReflection(userId: string, journeyDayId: string) {
 ...
 * Report a slip event
 */
export async function reportSlip(userId: string, data: SlipData) {
 ...
 * Get slip history
 */
export async function getSlipHistory(
  userId: string,
  limit: number,
  offset: number
) {
 ...
 */
export async function completeDayTasks(userId: string) {
...
// Helper functions
async function getTodayXPInternal(userId: string): Promise<number> {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
...
async function getRemainingXPToday(userId: string): Promise<number> {
  const todayProgress = await getTodayProgress(userId);
  const remainingTasks = todayProgress.tasks.filter(t => !t.completed).length;
  return remainingTasks * 10; // 10 XP per task
}

async function getNextBadgeInfo(userId: string) {
...
function calculateLevelFromXP(totalXP: number): { level: number; progress: number } {
  let level = 1;
  let xpRequired = 0;
  let nextLevelXP = 100;
