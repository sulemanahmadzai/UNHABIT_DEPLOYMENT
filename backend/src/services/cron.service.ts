...
async function getCron() {
    try {
        ...
        console.warn(
            "⚠️  node-cron is not installed. Cron jobs are disabled.\n" +
        ...
        for (const { user_id } of activeJourneys) {
            ...
            if (completedToday === 0) {
                await sendPushToUser(
                    user_id,
                    "📅 Today's Challenge",
                    "Today's Challenge is waiting! Complete it to keep your habit alive. 🎯",
        ...
        for (const { user_id, current_length } of streaks) {
            ...
            if (completedToday === 0) {
                await sendPushToUser(
                    user_id,
                    "🔥 Streak in Danger!",
                    `Keep your ${current_length}-day streak alive! Log your habit now before midnight. 🔥`,
                    { type: "streak_saver_warning", current_streak: current_length }
                );
                sent++;
            } else {
                skipped++;
