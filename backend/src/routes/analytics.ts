...
r.get("/consistency", requireAuth, async (req, res, next) => {
  try {
    const timeWindow = (req.query.time_window as string) || "weekly";
    const consistency = await AnalyticsService.getConsistencyIndex(
      req.user!.id,
      timeWindow
    );
...
r.get("/adherence/:journeyId", requireAuth, async (req, res, next) => {
  try {
    ...
    const scores = await AnalyticsService.getAdherenceScores(
      req.user!.id,
      journeyId
    );
