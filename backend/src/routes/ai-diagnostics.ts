...
r.post("/quiz-form", requireAuth, async (req, res, next) => {
  try {
    ...
    const diagnostic = await AIDiagnosticsService.storeQuizForm(
      req.user!.id,
      data.user_habit_id,
      data.raw_input,
      data.quiz_form
...
r.post("/quiz-summary", requireAuth, async (req, res, next) => {
  try {
    ...
    const diagnostic = await AIDiagnosticsService.storeQuizSummary(
      req.user!.id,
      data.user_habit_id,
      data.raw_input,
      data.quiz_summary,
...
r.post("/safety", requireAuth, async (req, res, next) => {
  try {
    ...
    const diagnostic = await AIDiagnosticsService.storeSafetyAssessment(
      req.user!.id,
      data.user_habit_id,
      data.raw_input,
      data.safety
...
r.post("/plan-21d", requireAuth, async (req, res, next) => {
  try {
    ...
    const diagnostic = await AIDiagnosticsService.store21DayPlan(
      req.user!.id,
      data.user_habit_id,
      data.raw_input,
      data.plan
...
r.get("/", requireAuth, async (req, res, next) => {
  try {
    const userHabitId = req.query.user_habit_id as string | undefined;
    const type = req.query.type as "quiz_form" | "quiz_summary" | "safety_assessment" | "plan_21d" | undefined;
...
r.get("/habit/:habitId/latest/:type", requireAuth, async (req, res, next) => {
  try {
    ...
    const diagnostic = await AIDiagnosticsService.getLatestDiagnostic(
      req.user!.id,
      habitId,
      type as "quiz_form" | "quiz_summary" | "safety_assessment" | "plan_21d"
    );
